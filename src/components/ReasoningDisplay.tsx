    ...
    <div className="text-sm text-left space-y-4 w-full">
      ...
      <div className="space-y-3">
        {breakdown.map((item, index) => (
          <div key={index} className="grid grid-cols-[1fr,auto,auto] items-center gap-x-4">
            <div className="text-muted-foreground text-xs">{item.description}</div>
            <div className="text-right font-mono text-sm px-4">
                <Equation text={item.calculation} />
            </div>
            <div className="text-right font-mono text-sm font-semibold">= {formatLargeNumber(item.value)}</div>
          </div>
        ))}
        ...
        <div className="grid grid-cols-[1fr,auto,auto] items-center gap-x-4 pt-2">
            ...
            <div></div>
            <div className="text-right font-bold text-primary text-lg">= {formatNumber(finalTurnover)}</div>
        </div>
      </div>
    </div>
  );
